...
class DictationSessionController:
    ...
    def start_session(self, label: str = "") -> SessionState:
        if self.state.is_active:
            return self.state
        cleaned = slugify_filename(label) if label.strip() else datetime.now().strftime("session_%Y%m%d_%H%M%S")
        self.state = SessionState(started_at=datetime.now(), label=cleaned)
        return self.state
    ...
    def stop_session(self) -> list[Path]:
        ...
        for export_path in sorted(self.config.macwhisper_export_path.glob("*.txt")):
            ...
            if modified_at < self.state.started_at:
                continue
            stem = f"{self.state.label}_{slugify_filename(export_path.stem)}"
